...
class ConnMongoDB:
    ...
    @staticmethod
    def readContentJson():
        fieldsPath = os.path.join(os.path.abspath(os.path.dirname(__file__)), "BaseData/contentCodes.json")
    ...
    def resultDict(reportCode):
        reportCodePath = os.path.join(os.path.abspath(os.path.dirname(__file__)), "responseJsonCodes/" + reportCode + ".json")
    ...
    @staticmethod
    def readFieldsJson():
        fieldsPath = os.path.join(os.path.abspath(os.path.dirname(__file__)), "BaseData/fieldsCodes.json")
        with open(fieldsPath, mode='r', encoding="UTF-8") as f:
    ...
    def resultCompare(fieldsJson, resultDict):
        for key in fieldsJson:
            if fieldsJson[key] == resultDict[key]:
                ...
                print("实时接口返回 {} 数据指标 {}:{} 存在差异".format(resultDict["reportCode"], key, fieldsJson[key]))
        if len(resultDict) != 1:
...
if __name__ == '__main__':
    ...
    for reportCode in reportCodeList:
        ...
        if len(resultDict) != 2086:
            print("异步差异数据: " + reportCode, len(conn.selectContent(reportCode)))
        # 判读content数据是否相等
        if contentJson != resultDict:
            print("mongo中 {} 数据结果不正确".format(reportCode))
        # 接口实时返回值对比
        responseFieldsJson = conn.resultDict(reportCode)
        ConnMongoDB.resultCompare(fieldsJson, responseFieldsJson)

    # mongo中总数据量
    print("数据总量" + str(conn.getCount()))  # 4047