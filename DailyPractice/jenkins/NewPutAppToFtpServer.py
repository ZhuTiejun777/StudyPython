...
class PutAppToFtpServer:
    ...
    app_dir = np.array([["dub-manage-webapps", "192.168.3.129",
                        ...
                        ["dub-webapps-yb", "192.168.8.27",
                         "/usr/local/tomcat/tomcat8_jdk1.8_8203_dub_webapps_yb/webapps/dub-webapps-yb.war",
                         "dub-webapps-yb.war"],
                        ...
                        ["dub-webapps", "192.168.3.228",
                         "/usr/local/tomcat/tomcat8_jdk1.8_8205_DUB_WEBAPPS/webapps/dub-webapps.war",
                         "dub-webapps.war"],
                        ...
                        ["dub-dleybsl-webapp", "192.168.8.27",
                         "/usr/local/tomcat/tomcat8_jdk1.8_8200_dub_dleybsl_webapp/webapps/dub-dleybsl-webapp.war",
                         "dub-dleybsl-webapp.war"],
                        ["dub-dlfreight-webapp", "192.168.8.27",
                         "/usr/local/tomcat/tomcat8_jdk1.8_8201_dub_dlfreight_webapp/webapps/dub-dlfreight-webapp.war",
                         "dub-dlfreight-webapp.war"],
                        ["dub-exchange-edl-webapp", "192.168.8.27",
                         "/usr/local/tomcat/tomcat8_jdk1.8_8202_dub_exchange_edl_webapp/webapps/dub-exchange-edl-webapp.war",
                        ...
                        ["dub-front-end-dl", "192.168.3.129", "/usr/local/tomcat/prod-dub-front-end-dl/dist_dl.tar",
                         "dist_dl.tar"],
                        ["dub-webapp-finance", "192.168.3.129", "/usr/local/tomcat/prod-dub-webapp-finance/dist_finance.tar",
                         "dist_finance.tar"],
                        ["dub-webapp-declare", "192.168.3.129", "/usr/local/tomcat/prod-dub-webapp-declare/dist_declare.tar",
                         "dist_declare.tar"],
                        ...
                        ["dub-user-app", "192.168.8.109", "/server/soft/exchange/deploy/dub-user-app/",
                         "dub-user-app.tar"],
                        ["dub-dfs2.0", "192.168.8.12", "/usr/local/tomcat/dfs-service-7210/dfs-service-1.0.0-SNAPSHOT.jar",
                         "dfs-service-1.0.0-SNAPSHOT.jar"],
        ...
        for i in range(app_dir.shape[0]):
            if app_dir[i][0] in self.app_list:
                ...
                if app_dir[i][2][-1] != "/":
                    ...
                    str1 = ssh_x.run_cmd("ls -lh " + app_dir[i][2])
                    str2 = bytes.decode(str1).split(' ')[-3]
                    if int(str2) == time.localtime().tm_mday:
                        ssh_x.download(app_dir[i][2], app_local_dir)
                        print("应用" + app_dir[i][0] + "拷贝到本地路径成功；")
                    else:
                        self.app_list.remove(app_dir[i][0])
                        print("请注意：" + app_dir[i][0] + "应用日期不是当天，故没有拷贝该应用；")
                else:
                    str1 = ssh_x.run_cmd("ls -lh " + app_dir[i][2] + "*.sh")
                    str2 = bytes.decode(str1).split(' ')[-3]
                    if int(str2) == time.localtime().tm_mday:
